# This script sends an email to the DS List Serve based on an XML document
# describing that email.
#
# This script depends on premailer.
# https://pypi.python.org/pypi/premailer

__author__ = 'Ellis Michael (r.ellis.michael@gmail.com)'
...
SERVER_EMAIL_FOLDER = 'http://web2.cns.utexas.edu/ds/weekly_email/emails/'

EDITOR_MAGIC_STRING = '*|EDITOR|*'
SUBJECT_MAGIC_STRING = '*|MC:SUBJECT|*'
DATE_MAGIC_STRING = '*|DATE|*'
TEASER_MAGIC_STRING = '*|MC:TEASER|*'
...
DS_LIST_SERVE_EMAIL = 'ds@utlists.utexas.edu'
...
def send_email(html_email, volume_no, issue_no):
  ...
  msg['Subject'] = "DS Weekly News: Volume %s, Issue %s" % (volume_no, issue_no)
  msg['From'] = "%s <%s>" % (senders_name, senders_email)
  msg['To'] = "Dean's Scholars <%s>" % DS_LIST_SERVE_EMAIL
  
  from textwrap import fill # Break lines to fix random !
  # TODO: clean this up
  part = MIMEText(fill(html_email, 800), 'html')
  msg.attach(part)
  ...
  try:
    ...
    server.sendmail(senders_email, DS_LIST_SERVE_EMAIL, msg.as_string())
  finally:
    server.quit()
...
def main(argv=sys.argv):
  ...
  print('Opening the email file for review.\n'
        ...
        'Please add the file to the GitHub repo as well.' %
        SERVER_EMAIL_FOLDER)
  time.sleep(1)
