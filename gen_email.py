# This script builds sends an email for the SPS List Serve based on an XML document
# describing that email.
#
# This script depends on premailer.
# https://pypi.python.org/pypi/premailer

__author__ = 'Ellis Michael (r.ellis.michael@gmail.com), modified by Evan Ott (evan.ott@utexas.edu)'
...
SERVER_EMAIL_FOLDER = 'https://utlists.utexas.edu/sympa/arc/sps/'
...
def generate_email_from_data(file_name, email_data):
  ...
  output = output.replace(SERVER_URL_MAGIC_STRING,
                          os.path.join(SERVER_EMAIL_FOLDER))
...
def main(argv=sys.argv):
  ...
  if raw_input('review? y/n ') in ['y', 'Y']:
